...
:: 输入起始编号，默认 1
set /p "start_number=请输入起始编号（默认 1）："
if "%start_number%"=="" set /a start_number=1
...
:: 移动所有.gif文件到临时文件夹
for /f "delims=" %%F in ('dir /b /a-d /o:d *.gif') do (
    move "%%F" "%temp_folder%\"
    if errorlevel 1 (
...
:: 从临时文件夹将文件重新命名移回到原目录
for /f "delims=" %%F in ('dir /b /a-d /o:d "%temp_folder%\*.gif"') do (
    call :rename_proc "%%~F"
)
